...
const BudgetMode = ({ projectId }: BudgetModeProps) => {
  ...
  const fetchItems = useCallback(async () => {
    ...
    if (!rooms || rooms.length === 0) { setItems([]); setLoading(false); return; }
    ...
    const { data: rlRows } = await supabase
      .from("room_links")
      .select("id, link_id, room_id, status")
    ...
    const budgetItems: BudgetItem[] = rlRows
      .map((rl) => {
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      {items.length === 0 ? (
        <p className="flex-1 flex items-center justify-center text-sm text-muted-foreground">
          No items with URLs across your rooms
        </p>
      ) : (
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.room_link_id} className="border-b border-border/50 hover:bg-secondary/30">
                  ...
                  <td className="px-4 py-2 text-muted-foreground">{item.room_name}</td>
                  <td className="px-4 py-2">
                    <input
